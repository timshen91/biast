...
func newArticleHandler(w http.ResponseWriter, r *http.Request) {
    ...
    if r.Method == "POST" {
        if err := checkVerification(sid, r); err != nil {
...
func newArticleAuth(a, old *Article) {
    ...
    func checkVerification(sid string, a *http.Request) error {
        ans := strings.TrimSpace(a.Form.Get("verification"))
        if !checkVerifiCode(sid, ans) {
            return errors.New("Verifition code error.")
        }
        return nil
    }
